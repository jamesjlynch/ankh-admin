...
<label>Delivery method<select id="delivery-method" name="delivery_method" required><option value="">Choose method</option><?php $selectedDelivery=$_POST['delivery_method']??$repeatDeliveryMethod;foreach($deliveryMethods as $method):?><option value="<?=e($method)?>" <?=$selectedDelivery===$method?'selected':''?>><?=e($method)?></option><?php endforeach;?></select></label>
...
<div class="edit-section"><span class="wizard-kicker">PRODUCT TYPE</span><input type="hidden" id="presentation" name="presentation" value="<?=e($editOrder['presentation']??'')?>"><div class="presentation-picker" aria-label="Choose product type"><button type="button" data-presentation="Pen"><strong>Pen</strong><span>Peptide price + £20</span></button><button type="button" data-presentation="Cartridge"><strong>Cartridge</strong><span>Peptide price</span></button><button type="button" data-presentation="Vial"><strong>Vial</strong><span>Peptide price</span></button></div></div>
...
function addProduct(row){if(!row)return;const q=row.querySelector('.quantity');row.hidden=false;if((Number(q.value)||0)<1)q.value=1;updateTotal();if(strengthPicker)strengthPicker.hidden=true;if(productSearch){productSearch.value='';productSearch.focus()}if(productResults){productResults.hidden=true;productResults.replaceChildren()}if(typeof saveDraftOrder==='function')saveDraftOrder()}
