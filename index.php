...
<?php else:?>
<div class="heading customer-heading"><div><h1>Customers</h1><p class="muted">Save customers here before they place an order, or view their order history.</p></div><button type="button" id="show-add-customer" class="customer-plus" aria-label="Add customer" title="Add customer" aria-expanded="false" aria-controls="add-customer-panel">+</button></div>
...
function showProductResults(){if(!productSearch||!productResults)return;const term=productSearch.value.toLowerCase().trim();productResults.replaceChildren();if(!term){productResults.hidden=true;return}const matches=productGroups().filter(group=>group.base.toLowerCase().includes(term)).slice(0,8);if(!matches.length){const empty=document.createElement('div');empty.className='product-result-empty';empty.textContent='No matching product';productResults.append(empty);productResults.hidden=false;return}matches.forEach(group=>{const b=document.createElement('button');b.type='button';b.className='product-result';b.setAttribute('role','option');const name=document.createElement('strong');name.textContent=group.base;const info=document.createElement('span');const strengths=group.rows.map(r=>r.dataset.productStrength).filter(Boolean);info.textContent=group.rows.length>1?(strengths.length?strengths.join(' · '):'Choose option'):'Add';b.append(name,info);b.addEventListener('click',()=>{if(group.rows.length===1)addProduct(group.rows[0]);else showStrengths(group)});productResults.append(b)});productResults.hidden=false}
