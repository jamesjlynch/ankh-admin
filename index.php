...
<?php if($view==='orders'): ?>
<div class="heading"><div><h1>Orders</h1><p class="muted">Every order, from enquiry to dispatch.</p></div><a class="button" href="?view=new">+ New order</a></div>
...
<?php elseif($view==='new'):?>
<h1>New order</h1><p class="muted">Record an order from WhatsApp or a customer enquiry.</p><form method="post" class="panel"><?php csrf();?><input type="hidden" name="action" value="order"><div class="two"><label>Customer name<input name="customer" maxlength="160" required autocomplete="name"></label><label>Phone<input name="phone" maxlength="40" type="tel" autocomplete="tel"></label></div><label>Delivery address<textarea name="address" maxlength="2000" autocomplete="street-address"></textarea></label><h2>Products</h2>
<?php foreach($products as $p):if(!$p['active'])continue;?><label class="product-pick"><span><?=e($p['name'])?><small><?=money($p['price'])?></small></span><input aria-label="<?=e($p['name'])?> quantity" class="quantity" data-price="<?=$p['price']?>" name="qty[<?=$p['id']?>]" type="number" min="0" max="999" value="0"></label><?php endforeach;?>
<?php if(!$products):?><p>Add products in the <a href="?view=products">Products tab</a> first.</p><?php endif;?>
<div class="line"><strong>Product subtotal</strong><strong id="subtotal">£0.00</strong></div><label>Order notes<textarea name="notes" maxlength="4000" placeholder="Delivery instructions, payment reference…"></textarea></label><button>Create order</button></form>
...
document.querySelectorAll('.quantity').forEach(q=>q.addEventListener('input',()=>{let total=0;document.querySelectorAll('.quantity').forEach(x=>total+=Math.max(0,Number(x.value)||0)*Number(x.dataset.price));document.querySelector('#subtotal').textContent=new Intl.NumberFormat('en-GB',{style:'currency',currency:'GBP'}).format(total/100)}));
