...
<div class="product-search-wrap"><label for="product-search">Find a product<input id="product-search" type="search" placeholder="e.g. BPC, Retatrutide, BAC Water…" autocomplete="off" aria-autocomplete="list" aria-controls="product-results"></label><div id="product-results" class="product-results" role="listbox" hidden></div></div>
<div id="selected-products">
<?php foreach($products as $p):if(!$p['active'])continue;$postedQty=(int)($_POST['qty'][$p['id']]??0);$postedPrice=$_POST['price'][$p['id']]??number_format((int)$p['price']/100,2,'.','');?><div class="product-pick<?=$postedQty>0?' picked':''?>" data-product-id="<?=$p['id']?>" data-product-name="<?=e(strtolower($p['name']))?>" data-product-label="<?=e($p['name'])?>" data-standard-price="<?=e(number_format((int)$p['price']/100,2,'.',''))?>" <?=$postedQty>0?'':'hidden'?>><span><?=e($p['name'])?><small>Standard <?=money($p['price'])?></small></span><div class="stepper"><button type="button" data-change="-1" aria-label="Remove one <?=e($p['name'])?>">−</button><input inputmode="numeric" aria-label="<?=e($p['name'])?> quantity" class="quantity" name="qty[<?=$p['id']?>]" type="number" min="0" max="999" value="<?=$postedQty?>"><button type="button" data-change="1" aria-label="Add one <?=e($p['name'])?>">+</button></div><label class="order-price">Price each for this order (£)<input class="line-price" name="price[<?=$p['id']?>]" type="number" min="0" max="100000" step=".01" inputmode="decimal" value="<?=e($postedPrice)?>"></label></div><?php endforeach;?>
</div>
...
const productSearch=document.querySelector('#product-search'),productResults=document.querySelector('#product-results'),productRows=[...document.querySelectorAll('[data-product-id]')],selectedEmpty=document.querySelector('#selected-empty');
...
function addProduct(row){if(!row)return;const q=row.querySelector('.quantity');row.hidden=false;if((Number(q.value)||0)<1)q.value=1;updateTotal();if(productSearch){productSearch.value='';productSearch.focus()}if(productResults){productResults.hidden=true;productResults.replaceChildren()}}
function showProductResults(){if(!productSearch||!productResults)return;const term=productSearch.value.toLowerCase().trim();productResults.replaceChildren();if(!term){productResults.hidden=true;return}const matches=productRows.filter(row=>row.dataset.productName.includes(term)).slice(0,8);if(!matches.length){const empty=document.createElement('div');empty.className='product-result-empty';empty.textContent='No matching product';productResults.append(empty);productResults.hidden=false;return}matches.forEach(row=>{const b=document.createElement('button');b.type='button';b.className='product-result';b.setAttribute('role','option');const name=document.createElement('strong');name.textContent=row.dataset.productLabel;const price=document.createElement('span');price.textContent='£'+Number(row.dataset.standardPrice).toFixed(2);b.append(name,price);b.addEventListener('click',()=>addProduct(row));productResults.append(b)});productResults.hidden=false}
productSearch?.addEventListener('input',showProductResults);
productSearch?.addEventListener('focus',showProductResults);
document.addEventListener('click',e=>{if(productResults&&!e.target.closest('.product-search-wrap'))productResults.hidden=true});
